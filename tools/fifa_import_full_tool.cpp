...
ImportStats importTeamData(const std::string &clubsCsvPath, const std::string &playersCsvPath,
    ...
    for (auto &[clubId, players] : clubPlayers) {
        ...
        // Validate minimum of 2 goalkeepers per club
        if (goalkeepers.size() < 2) {
            auto clubIt = clubs.find(clubId);
            std::string clubName = (clubIt != clubs.end()) ? clubIt->second.name : "Unknown";
            throw std::runtime_error("Club '" + clubName + "' (id: " + std::to_string(clubId) +
                                     ") has only " + std::to_string(goalkeepers.size()) +
                                     " goalkeeper(s). Minimum 2 required.");
        }
